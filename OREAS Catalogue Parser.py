#   OREAS Catalogue Parser v1.2 by ZH (06/04/2021)
...
def getInput():                             # Gets user input and fills related variables accordingly
    ...
    global crmID
    global crmNum
    crmInput = input("Enter CRM ID (eg. 72b or 920): ")
    crmID = crmPrefix.format(crmInput)
    crmNum = [crmInput]
...
def fillValues(crm):                        # Fills all 'values' lists with the converted data scraped from catalogue then fills the outputArray list
    ...
    with open(catalogueName, mode='r') as oreas_file:            #opens oreas catalogue file as a csv dictionary for improved lookup
    ...
    with open(catalogueName, mode='r') as oreas_file:
        reader = csv.DictReader(oreas_file, delimiter=',')
        for row in reader:
            if row["CRM ID"] == crm:
